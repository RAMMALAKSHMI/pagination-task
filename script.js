...
function showTable(pageNumber) 
{
  ...
  headLabels
    ...
    .map((cell) => trow.append(cell)); 
  
  /*for(var k=0;k<headLabels.length;k++){
    let cell = document.createElement("th");
    cell.innerText = headLabels[k];
  ...
  for (let i = (pageNumber - 1) * 5; i < data.length; i++) 
  {
    if (i >= pageNumber * 5) 
    {
      break;
  ...
  var tabDiv = document.getElementById("cont");
  tabDiv.innerHTML = "";
  tabDiv.appendChild(table);
}

document.onload = showTable(1);
